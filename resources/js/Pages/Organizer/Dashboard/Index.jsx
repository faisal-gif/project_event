...
export default function OrganizerDashboard({ auth, totalEvents, totalRevenue, ticketsSold, recentEvents }) {
    ...
    return (
        <AuthenticatedLayout
            user={auth.user}
            header={<h2 className="font-semibold text-xl text-base-content leading-tight">Organizer Dashboard</h2>}
        >
                            ...
                            <div className="overflow-x-auto">
                                <table className="min-w-full divide-y divide-gray-200">
                                    <thead className="bg-base-200">
                                        <tr>
                                            <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-base-content/60 uppercase tracking-wider">
                                                Title
                                            </th>
                                            <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-base-content/60 uppercase tracking-wider">
                                                Category
                                    ...
                                    <tbody className="bg-base-100 divide-y divide-gray-200">
                                        {recentEvents.length > 0 ? recentEvents.map((event) => (
                                            <tr key={event.id}>
                                                ...
                                                <td className="px-6 py-4 whitespace-nowrap">
                                                    <div className="text-sm text-base-content">{new Date(event.start_date).toLocaleDateString()}</div>
                                                </td>
                                                <td className="px-6 py-4 whitespace-nowrap">
                                            ...
                                            <tr>
                                                <td colSpan="4" className="px-6 py-4 text-center text-sm text-base-content/60">
